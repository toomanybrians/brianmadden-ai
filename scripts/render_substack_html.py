...
Only pulls four pieces from the canon file — not the whole thing:
  1. The YouTube URL (plain text, its own paragraph)
  2. Description
  3. Links mentioned
  4. Transcript
No platform-links block beyond YouTube, no Topics-covered bullets, no
Chapters list — Brian's explicit call (2026-09-02), not an oversight.
"""
import argparse
import html
import re
...
def render_episode(md_path: Path) -> Path:
    ...
    parts = []
    if youtube_url:
        parts.append(f"<p>{html.escape(youtube_url)}</p>")
    parts.append(description_html)
    parts.append("<h3>Links mentioned</h3>")
    parts.append(links_html)
    parts.append("<h3>Transcript</h3>")
